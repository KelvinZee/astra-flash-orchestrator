...
description: Plan and execute substantial multi-file builds, features, migrations, and refactors with Astra as orchestrator and an explicitly pinned native-v2 Codex implementation worker. The default worker in this fork is gpt-5.6-luna; alternate workers may be selected only when the local catalog advertises native v2 subagent support. Use for phased planning and delegated build execution, including existing Superpowers or GSD plans. Skip trivial edits and explicitly single-agent tasks. Worker children must not invoke this orchestration skill.
---

# Astra plans. The pinned builder implements. Astra accepts.
...
contract is ready, the pinned builder owns repository discovery needed within the brief,
...
and what the builder will implement. These routing decisions are not alternate modes.
...
the builder under a vague "build it" prompt.
...
bundle per phase when its contract is stable. Each builder assignment has exact
...
stay with the builder; Astra does not write the entire implementation in the plan. Long
...
Default to ONE active builder writer in the current workspace. A native subagent
...
subagents would also route to the pinned worker, so do not mistake a default child for an
