...
Use one configured builder writer by default; do not create an agent for each tiny coding step.
...
the configured builder a coherent end-to-end phase bundle and let it own repository discovery,
...
A builder child executes its assigned brief; it must not load the orchestration
...
Do not switch the root to the worker, silently fall back to a different worker model,
launch another agent CLI, loosen permissions, expose secrets, auto-commit,
push, deploy, or start paid setup smoke tests. Normal delegated implementation uses the worker route explicitly pinned by the installer. The default in this fork is native `gpt-5.6-luna`; alternate workers must be explicitly selected and must advertise native v2 subagent support. Obey the user's data-sharing and spending restrictions. Installation is not evidence of a successful routed model request.
